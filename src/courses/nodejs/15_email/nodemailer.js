...
http
  .createServer(function (zahtev, odgovor) {
    ...
    if (zahtev.url == "/mejl" && zahtev.method == "POST") {
      ...
      odgovor.write(
        'Ime i prezime: <input type="text" name="ime" id="ime"><br>'
      );
      odgovor.write(
        'Mejl adresa: <input type="text" name="mejl" id="mejl"><br>'
      );
      odgovor.write('Subject: <input type="text" name="sub" id="sub"><br>');
      odgovor.write(
        'Sadržaj poruke: <textarea id="poruka" name="poruka" rows="4" cols="50">Unesite poruku</textarea><br>'
      );
