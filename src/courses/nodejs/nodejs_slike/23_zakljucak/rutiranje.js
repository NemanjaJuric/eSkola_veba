...
function kreirajHTTPZaglavlje(cookie, da) {
  ...
  if (da) {
    httpZaglavlje = {
      ...
      "Set-Cookie":
        cookie.name + "=" + cookie.value + ";max-age=" + cookie.max_age,
    };
  } else {
    httpZaglavlje = {
      "Content-Type": "text/html; charset=utf-8",
...
function kreirajPocetakStr(active) {
  var str =
    '<html><head><title>NodeJS elektronski kurs</title><link rel="stylesheet" href="stil.css"></head><body><div class="content">';

  str +=
    '<div class="header"><h1/>Elektronski kurs NodeJS - Virtuelna biblioteka</h1>';
  str += '<ul class="menu">';

  if (active == 1)
    str += '<li><a href="/index" class="active">Sve knjige</a></li>';
  else str += '<li><a href="/index">Sve knjige</a></li>';

  if (active == 2)
    str += '<li><a href="/nova_knjiga"  class="active">Dodaj knjigu</a></li>';
  else str += '<li><a href="/nova_knjiga">Dodaj knjigu</a></li>';

  if (active == 3)
    str += '<li><a href="/izmeni" class="active">Izmeni knjigu</a></li>';
  else str += '<li><a href="/izmeni">Izmeni knjigu</a></li>';

  if (active == 7)
    str += '<li><a href="/obrisi" class="active">Obriši knjigu</a></li>';
  else str += '<li><a href="/obrisi">Obriši knjigu</a></li>';

  if (active == 4)
    str += '<li><a href="/registracija" class="active">Registracija</a></li>';
  else str += '<li><a href="/registracija">Registracija</a></li>';

  if (active == 5) str += '<li><a href="/login" class="active">Login</a></li>';
  else str += '<li><a href="/login">Login</a></li>';

  if (active == 6)
    str += '<li><a href="/logout" class="active">Logout</a></li>';
  else str += '<li><a href="/logout">Logout</a></li>';
  ...
  if (stranica === "/obrisi") {
        ...
        if (typeof id != "undefined" && id != "") {
          sql.izvrsi(sql.obrisiKnjigu(id), function (redovi) {
            if (redovi == null || redovi.affectedRows < 1) {
              ...
              res.write(
                'ID knjige: <input type="text" id="id" name="id" /><br/>'
              );
      ...
      if (session) {
        if (typeof id != "undefined" && id != "") {
          sql.izvrsi(sql.nadjiKnjigu(id), function (redovi) {
            ...
            res.write(
              'Naslov: <input type="text" id="naslov" name="naslov" value="' +
                redovi[0].naslov +
                '"/><br/>'
            );
            res.write(
              'Autor: <input type="text" id="autor" name="autor" value="' +
                redovi[0].autor +
                '"/><br/>'
            );
            res.write(
              'Izdavač: <input type="text" id="izdavac" name="izdavac" value="' +
                redovi[0].izdavac +
                '"/><br/>'
            );
            res.write(
              'Žanr: <input type="text" id="zanr" name="zanr" value="' +
                redovi[0].zanr +
                '"/><br/>'
            );
            res.write(
              'Godina izdanja: <input type="text" id="gi" name="gi" value="' +
                redovi[0].god_izdanja +
                '"/><br/>'
            );
            res.write(
              'Broj strana: <input type="text" id="bs" name="bs" value="' +
                redovi[0].br_str +
                '"/><br/>'
            );
            res.write(
              'Tiraž: <input type="text" id="tiraz" name="tiraz" value="' +
                redovi[0].tiraz +
                '"/><br/>'
            );
            res.write(
    ...
    if (metod == "GET") {
      sql.izvrsi(sql.sveKnjige(), function (redovi) {
        ...
        res.write("<tr>");
        res.write(
          "<th>ID</th><th>Naslov</th><th>Autor</th><th>Broj strana</th><th>Žanr</th><th>Izdavač</th><th>Godina izdanja</th><th>Tiraž</th>"
        );
        res.write("</tr>");
    ...
    if (metod === "GET") {
      if (session) {
        ...
        res.write(
          'Šifra: <input type="password" id="pass" name="pass" /><br/>'
        );
    ...
    if (metod === "GET") {
      if (session) {
        ...
        res.write(
          '<lagel for="ime">Ime:</label><input type="text" id="ime" name="ime" /><br/>'
        );
        res.write(
          '<lagel for="prezime">Prezime:</label><input type="text" id="prezime" name="prezime" /><br/>'
        );
        res.write(
          '<lagel for="user">Korisničko ime:</label><input type="text" id="user" name="user" /><br/>'
        );
        res.write(
          '<lagel for="pass">Šifra:</label><input type="password" id="pass" name="pass" /><br/>'
        );
        res.write(
          '<lagel for="mejl">Mejl:</label><input type="text" id="mejl" name="mejl" /><br/>'
        );
        res.write(
          '<lagel for="god">Godište:</label><input type="text" id="god" name="god" /><br/>'
        );
    ...
    if (metod == "GET") {
      if (session) {
        ...
        res.write(
          'Naslov: <input type="text" id="naslov" name="naslov" /><br/>'
        );
        res.write('Autor: <input type="text" id="autor" name="autor" /><br/>');
        res.write(
          'Izdavač: <input type="text" id="izdavac" name="izdavac" /><br/>'
        );
        res.write('Žanr: <input type="text" id="zanr" name="zanr" /><br/>');
        res.write(
          'Godina izdanja: <input type="text" id="gi" name="gi" /><br/>'
        );
